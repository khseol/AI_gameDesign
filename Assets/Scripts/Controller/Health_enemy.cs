...
public class Health_enemy : MonoBehaviour
{
    ...
    GhostState_enemy ghostStateScript;

    void Start()
    {
        noDamage_script = gameObject.GetComponent<Invincibility>();
        ghostStateScript = gameObject.GetComponent<GhostState_enemy>();
    }
    ...
    {
        if (onComing.gameObject.tag == "burst" && noDamage_script.enabled == false && health > 0)
        {
    ...
    {
        Debug.Log("Enemy hit! apply damage and apply score");
        health = health - damage;
        ScoreScript.scoreValue = ScoreScript.scoreValue + 500;
    }
